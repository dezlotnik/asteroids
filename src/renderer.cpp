...
void Renderer::RenderGameObject(const GameObject *game_object,
  ...
  destination.x =
      static_cast<int>(game_object->getPose().x - game_object->getWidth() / 2);
  destination.y =
      static_cast<int>(game_object->getPose().y - game_object->getHeight() / 2);
  ...
  SDL_RenderCopyEx(sdl_renderer, texture, NULL, &destination,
                   game_object->getPose().yaw, NULL, flip);
