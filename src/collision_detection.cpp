...
bool CollisionDetection::detect_collision(const GameObject &game_object_1, const GameObject &game_object_2) {
  ...
  if (game_object_1.getPose().x - game_object_1.getWidth() / 2 >=
          game_object_2.getPose().x + game_object_2.getWidth() / 2 ||
      game_object_2.getPose().x - game_object_2.getWidth() / 2 >=
          game_object_1.getPose().x + game_object_1.getWidth() / 2) {
    return false;
  }
  if (game_object_1.getPose().y + game_object_1.getHeight() / 2 <=
          game_object_2.getPose().y - game_object_2.getHeight() / 2 ||
      game_object_2.getPose().y + game_object_1.getHeight() / 2 <=
          game_object_1.getPose().y - game_object_1.getHeight() / 2) {
    return false;
  }
  return true;
}
